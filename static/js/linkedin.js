...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnDownload.addEventListener("click", () => {
    if (!currentJobId) return;
    window.location.href = `/api/linkedin/download/${currentJobId}`;
  });
  ...
  function renderProfiles(leads) {
    ...
    if (leads.length === 0) {
      profilesBody.innerHTML = `<tr><td colspan="7" class="text-center text-muted py-4">No results found.</td></tr>`;
      return;
    }
    leads.forEach((l, idx) => {
      const profileLink = l.profile_url && l.profile_url !== "N/A"
  ...
  function renderCompanies(leads) {
    ...
    leads.forEach((l, idx) => {
      const companyLink = l.company_url && l.company_url !== "N/A"
  ...
  function setFormEnabled(enabled) {
    ...
    if (enabled) {
      ...
      btnScrape.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Working...';
    }
  }
